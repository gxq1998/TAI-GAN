...
cardiac_data = EarlyToLateDatasetWithRVLVTemp_all(data_dir, eq_key = 'curve_labeled_eq', cv_split_index=train_indexes, norm=True, transform=transform, thre=thre) #, pad = np.zeros((128,128,17)))
...
cardiac_eval_data = EarlyToLateDatasetWithRVLVTemp_all(data_eval_dir, eq_key = 'curve_labeled_eq', cv_split_index=val_indexes, norm=True, pad = np.zeros((64,64,12)), thre=thre) #, transform=transform)

data_eval_loader = torch.utils.data.DataLoader(dataset=cardiac_eval_data,
                                          batch_size=1,
                                          shuffle=False)

#cardiac_eval_crop_data = EarlyToLateDatasetWithRVLVTemp(data_eval_dir, eq_key = 'curve_labeled_eq', cv_split_index=val_indexes, norm=True, transform=transform, thre=thre)
...
for epoch in range(init_epoch,num_epochs):
    ...
    for i, (img, last_frame, _,_) in enumerate(data_loader):
        ...
        G_sim_losses.append(G_mse_loss.data)

        print('Epoch [%d/%d], Step [%d/%d], D_loss: %.4f, G_loss: %.4f, G_sim_loss: %.4f'
              % (epoch+1, num_epochs, i+1, len(data_loader), D_loss.data, G_loss.data, G_mse_loss))
    ...
    if (epoch + 1) % eval_interval == 0:
        ...
        calculate_mse_and_ssim(G, data_eval_loader, epoch, num_epochs, device, save=True, save_dir=save_dir + 'mask_eval_whole_ssim_test')
