...
from utils import *
from cardiac_dataset_one2one import *
...
save_name = 'tempcode_mse_film_rvlvcurvetmp_highD_trans5_dloss_allto27_lowDlr_randomcroprotateshift_cardiaceval_imgnorm1_avgpool_batch4_{0}/'.format(str(cv_fold))
...
img, last_frame, curves, _ = next(iter(data_eval_loader))
inshape = img.shape[2:]
infeats = img.shape[1]
...
if init_epoch:
    step = len(cardiac_eval_data)*init_epoch
else:
    step = 0
...
for epoch in range(init_epoch,num_epochs):
    ...
    if (epoch + 1) % eval_interval == 0:
        ...
        #plot_result(G, data_eval_loader, epoch, num_epochs, device, save=True, save_dir=save_dir + 'all_on_eq-1_eval_whole_ssim')
        calculate_mse_and_ssim(G, data_eval_loader, epoch, num_epochs, device, save=True, save_dir=save_dir + 'all_on_eq+1_eval_whole_ssim')
        #save_result(G, data_eval_loader, epoch, device, save=True, save_dir=save_dir + 'eval_whole_')
